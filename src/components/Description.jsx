...
const Description = ({ addToCart }) => {
  ...
  const handleViewMore = useCallback(() => {
    setShownMeals((prevShownMeals) => {
      ...
      const filteredMeals = meals.filter(meal => meal.category === category);
      return filteredMeals.slice(0, Math.min(newShownCount, filteredMeals.length));
    });
  }, [meals, category]);

  const showMoreButtonVisible = shownMeals.length < meals.filter(meal => meal.category === category).length;
        ...
        <div className="flex flex-col items-center justify-center gap-10 py-36 px-30">
          ...
          <div className="flex w-full flex-col justify-center items-center gap-10">
            <div className="flex gap-8">
              <Button onClick={() => handleCategorySelection('Dessert')} variant={category === 'Dessert' ? 'default' : 'blank'}>Dessert</Button>
              <Button onClick={() => handleCategorySelection('Dinner')} variant={category === 'Dinner' ? 'default' : 'blank'}>Dinner</Button>
              <Button onClick={() => handleCategorySelection('Breakfast')} variant={category === 'Breakfast' ? 'default' : 'blank'}>Breakfast</Button>
            </div>
